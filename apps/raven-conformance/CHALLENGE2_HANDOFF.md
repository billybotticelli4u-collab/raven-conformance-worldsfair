> **HISTORICAL, NON-OPERATIVE.** Retained because the shipped test suite reads it. Paths, bundles and states it names are from an earlier internal lane and are not instructions. The only current public cold-start authority is the root `DEVELOPER.md`.
...
- Canonical lock: `<historical-build-machine-path>` (+ `challenge2-judge-ui-artifacts.tgz`)
...
- Also: `<historical-build-machine-path>`
